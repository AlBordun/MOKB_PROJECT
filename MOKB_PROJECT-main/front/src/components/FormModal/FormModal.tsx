...
const FormModal: React.FC<FormModalProps> = ({
    ...
    const [insurancePolicyError, setInsurancePolicyError] = useState(false);
    ...
    const [vmpGroupError, setVmpGroupError] = useState(false);
            ...
            try {
                ...
                if (patientResponse.data && patientResponse.data.id) {
                    ...
                    const finalFormDataToSend = {
                        ...
                        // Обновляем или устанавливаем patient_id
                    };
                    console.log("Patient ID: ", patientResponse.data.id)
                    console.log("Отправляемые данные формы:", finalFormDataToSend);
                    ...
                    formDataToSend.form_name = radioGroupValue === 'Протокол' ? '1' :
                        radioGroupValue === 'Направление' ? '2' : '3';

                    const formResponse = finalFormDataToSend?.id
                        ? await axios.put(`http://localhost:8080/api/forms/update/${finalFormDataToSend.id}`, formDataToSend)
                        : await axios.post('http://localhost:8080/api/forms/submit', finalFormDataToSend)
                            .then(formResponse => {
    ...
    const handlePreview = async () => {
        if (patientData && formData) {
            ...
            console.log("Отправленные данные для предпросмотра:", patientAndFormDTO)

            try {
                const response = await axios.post('http://localhost:8080/api/export/preview', patientAndFormDTO, {responseType: 'blob'});
                console.log("Ответ сервера на запрос предпросмотра:", response)
    ...
    const handleChangeTextNumbers = (
        ...
        // Установка ошибки в зависимости от поля
        if (fieldName === 'snils') {
            setSnilsError(!isDigits);
        } else if (fieldName === 'insurance_policy') {
            setInsurancePolicyError(!isDigits);
        } else if (fieldName === 'passport_series') {
            setPassportSeriesError(!isDigits);
        } else if (fieldName === 'passport_number') {
            setPassportNumberError(!isDigits);
        } else if (fieldName === 'vmp_group') {
            setVmpGroupError(!isDigits);
        }
    ...
    const handleDateChange = (newDate: Dayjs | null) => {
        setPatientData(prevState => {
            ...
            return {
                ...prevState, // Копирование всех существующих полей prevState
                date_of_hosp: newDate ? newDate.format('YYYY-MM-DD') : null, // Обновление только поля date_of_hosp
                // Убедитесь, что здесь нет полей, которых нет в типе Patient
            };
        });
    };
                ...
                <Grid container sx={{
                    ...
                    <Grid item xs={12} sm={6} sx={{
                        ...
                        <LocalizationProvider dateAdapter={AdapterDayjs}>
                            <DateField
                                label="Дата рождения"
                                value={patientData?.date_of_birth ? dayjs(patientData.date_of_birth) : null} // Преобразование строки в Dayjs
                    ...
                    <Grid item xs={12} sm={6} sx={{
                        ...
                        <TextField label="Снилс" variant="outlined" fullWidth
                                   ...
                                   onChange={(e) => handleChangeTextNumbers(e, 'snils')}
                                   error = {snilsError}
                        />

                        <TextField label="Полис" variant="outlined" fullWidth
                                   value={patientData?.insurance_policy || ''}
                                   onChange={(e) => handleChangeTextNumbers(e, 'insurance_policy')}
                        ...
                        <Grid item xs={6}>
                            ...
                            <LocalizationProvider dateAdapter={AdapterDayjs}>
                                <DateField
                                    ...
                                    value={formData?.date_of_hosp ? dayjs(formData.date_of_hosp) : null} // Преобразование строки в Dayjs
                                    onChange={handleDateChange}
                                    format="DD-MM-YYYY"
                                />
                            </LocalizationProvider>
                    ...
                    <div className="flex flex-row items-center justify-start ml-4 p-3">
                        ...
                        >
                            ...
                            <Grid item xs={6}>
                                ...
                                <LocalizationProvider dateAdapter={AdapterDayjs}>
                                    <DateField
                                        label="Дата рождения"
                                        value={formData?.accomp_date_of_birth ? dayjs(formData.accomp_date_of_birth) : null} // Преобразование строки в Dayjs
                                        onChange={handleDateChange}
                                        format="DD-MM-YYYY"
                                    />
                                </LocalizationProvider>
