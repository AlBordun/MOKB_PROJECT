...
const NewPatientsTable: React.FC<newPatientsTableProps> = ({
    ...
    const filterField = (fieldValue: string | null | undefined, searchText: string): boolean => {
        if (fieldValue !== undefined && fieldValue !== null) {
            // Преобразование fieldValue в строку перед вызовом toLowerCase
    ...
    const filteredPatients = patients.filter((patient) => {
        ...
        const matchesSearch =
            filterField(patient.id ? patient.id.toString() : '', searchText) ||
            ...
            filterField(patient.gender ? patient.gender.toString() : '', searchText) ||
            filterField(patient.population_category ? patient.population_category.toString() : '', searchText) ||
    ...
    const handleCreateDocument = (patient: Patient) => {
                    ...
                    <TableBody sx={{
                        ...
                        {sortedAndFilteredPatients.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((patient) => (
                            <React.Fragment key={patient.id}>
                                ...
                                <TableRow>
                                    <TableCell style={{paddingBottom: 4, paddingTop: 0}} colSpan={6} >
                                        <Collapse in={openRows[patient.id]} timeout="auto" unmountOnExit>
                                            <Box margin={1}>
                                                ...
                                                <Button
                                                    variant="contained"
                                                    onClick={() => handleCreateDocument(patient)}
                                                >Новый документ
                                                </Button>
                                                    ...
                                                    <TableBody >
                                                        {patientForms[patient.id]?.map((form) => (
                                                            <TableRow key={form.id}>
                                                                ...
                                                                >
                                                                    <Checkbox
                                                                        checked={localSelected.includes(form.id)}
                                                                        onClick={(event) => handleClick(event, form.id)}
                                                                ...
                                                                <TableCell>{form.profile_name}</TableCell>
                                                                <TableCell>{form.date_of_hosp}</TableCell>
                                                            </TableRow>
                                                        ))}
                                                    </TableBody>
                                                </Table>
