...
const NewPatientsTable: React.FC<newPatientsTableProps> = ({
    ...
    const fetchPatientForm = async (patient_id: number) => {
        try {
            const response = await axios.get(`http://localhost:8080/api/forms/patient/${patient_id}`);
            // предполагается, что API возвращает массив форм с их кодами
            setPatientForms((prevForms) => ({ ...prevForms, [patient_id]: response.data }));
        } catch (error) {
                                                    ...
                                                    <TableBody>
                                                        {patientForms[patient.id]?.map((form) => (
                                                            <TableRow key={form.id}>
                                                                <TableCell>
                                                                    {form.form_name === '1' && (
                                                                        <Button variant="outlined" onClick={() => handleFormOpenModal(form, patient)}>
                                                                            Направление
                                                                        </Button>
                                                                    )}
                                                                    {form.form_name === '2' && (
                                                                    ...
                                                                    {/* Добавьте аналогичный код для других кодов из справочника */}
                                                                </TableCell>
