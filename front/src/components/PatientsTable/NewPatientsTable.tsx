...
const NewPatientsTable: React.FC<newPatientsTableProps> = ({
    ...
    const handleRowClick = async (patient_id: number | string | any) => {
        ...
        const selected = patients.find(patient => patient.id === patient_id);
...
console.log("SetSelected:", setSelectedPatient)
    ...
    //     if (selectedPatient) {
    //         // Здесь вы можете устанавливать начальные данные для формы в модальном окне,
    //         // если это необходимо. В противном случае, функция может оставаться пустой.
    //     }
    // };

    // const handleOpenModal = (form) => {
    //     // Здесь установите состояние для открытия модального окна
    //     // и передайте в него данные формы, например:
    //     setModalData(form);
    //     setOpenModal(true);
    // };
                        ...
                        {sortedAndFilteredPatients.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((patient) => (
                            <React.Fragment key={patient.id}>
                                <TableRow
                                    ...
                                    // onClick={(event) => handleRowClick(patient.id)}
                                    role="checkbox"
                                    ...
                                    <TableCell style={{paddingBottom: 0, paddingTop: 0}} colSpan={6}>
                                        <Collapse in={openRows[patient.id]} timeout="auto" unmountOnExit>
                                            <Box margin={1}>
                                                ...
                                                {/*                    </Button>*/}
                                                {/*                </TableCell>*/}
                                                {/*                /!*<TableCell>{form.creation_date}</TableCell>*!/*/}
                                                {/*            </TableRow>*/}
                                                {/*        ))}*/}
                                                {/*    </TableBody>*/}
                                                {/*</Table>*/}
