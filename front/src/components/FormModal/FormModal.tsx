...
const FormModal: React.FC<FormModalProps> = ({show, onClose, onSave,onSaveData, patient,form}) => {
    ...
    // const [MKB10Value,setMKB10Value] = React.useState<MKB10 | null>(mkb10)
    ...
    // useEffect(() => {
    //     axios.get(`http://localhost:8080//api/mkb10s/find_all_mkb10s}`)
    //         .then(response => {
    ...
    const transformFormData = (formData: Patient): Patient => {
        return {
            ...
            // ... Добавьте другие поля, которые нужно преобразовать
        };
    };
