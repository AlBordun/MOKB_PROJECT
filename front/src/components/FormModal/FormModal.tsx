...
const FormModal: React.FC<FormModalProps> = ({show, onClose, onSave, patient, copyData}) => {
    ...
    // useEffect(() => {
    //     const fetchData = async (tags: number[]) => {
    //         try {
    //             const tagsQueryParam = tags.map(tag => `tags[]=${tag}`).join('&')
    //             const responses = await Promise.all(tags.map(tag => axios.get(`http://localhost:8080/api/reference/ref_listss?${tagsQueryParam}`))
    //             );
    //
    //             console.log('Response data:', responses); //лог для отображения ответа сервера
    //
    //             const newData = {};
    //             console.log('newData:', newData);
    //             responses.forEach((response, index) => {
    //                 newData[tags[index]] = response.data.map((item: OptionItem) => item.text);
    //             })
    //
    //             // data.forEach((item, index) => {
    //             //     console.log(`Item ${index}:`, item);
    //             // });
    //             console.log('Data:', newData); // Добавьте лог для отображения обработанных данных
    //
    //             setDataByTag(newData);
    //             // const optionLabels = data.map(item => typeof item === 'string' ? item : '');
    //
    //             console.log('Option labels before set:', newData); // Добавьте лог перед установкой нового состояния
    //
