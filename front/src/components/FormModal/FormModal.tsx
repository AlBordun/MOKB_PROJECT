...
// Фетч для тэгов
    ...
    // useEffect(() => {
    //     axios.get<number[]>('http://localhost:8080/api/institution_name/names')
    //         .then(response => setInstitutionNames(response.data))
    //         .catch(error => console.error('Ошибка при загрузке данных:', error));
    // }, []);
    useEffect(() => {
        axios.get<{inst_id : number, name: string}[]>('http://localhost:8080/api/institution_name/all')
            .then(response => {
    ...
    // const handleInstitutionNamesChange = (event: React.SyntheticEvent, newValue: number | null) => {
    //     setSelectedinstitutionNames(newValue);
    //     setFormData(prevFormData => {
    //         // Проверяем, не является ли prevFormData null перед обновлением
    //         return prevFormData ? { ...prevFormData, inst_name: newValue } : null;
    //     });
    // };
    // const handleInstitutionNamesChange = (event: React.SyntheticEvent, newValue: number | null) => {
    //     const selectedInstitution = institutions.find(inst => inst.inst_id === newValue);
    //     setFormData(prevFormData => {
    //         return prevFormData ? { ...prevFormData, instName: selectedInstitution ? selectedInstitution.inst_id  : null } : null;
    //     });
    // };
    const handleInstitutionNamesChange = (event: React.SyntheticEvent, newValue: string | null) => {
        const selectedInstitution = institutions.find(inst => inst.name === newValue);
        setFormData(prevFormData => {
            return prevFormData ? { ...prevFormData, inst_name: selectedInstitution ? selectedInstitution.inst_id : null } : null;
        });
    };
    // useEffect(() => {
                ...
                <Grid container sx={{
                    ...
                    >
                        ...
                        <Grid item xs={6}>
                            ...
                            {/*    value={selectedinstitutionNames}*/}
                            ...
                            {/*    disablePortal*/}
                            {/*    id="institution-name-autocomplete"*/}
                            {/*    options={institutions.map(inst => inst.inst_id)}*/}
                            {/*    // value={formData ? institutions.find(inst => inst.id === formData.inst_name)?.instName : ''}*/}
                            {/*    value={*/}
                            {/*        formData && institutions.some(inst => inst.inst_id  === formData.inst_name)*/}
                            {/*            ? institutions.find(inst => inst.inst_id  === formData.inst_name)?.instName*/}
                            {/*            : ''*/}
                            {/*    }*/}
                            {/*    onChange={handleInstitutionNamesChange}*/}
                            {/*    renderInput={(params) => (*/}
                            ...
                            <Autocomplete
                                ...
                                value={
                                    formData ? institutions.find(inst => inst.inst_id === formData.inst_name)?.name : ''
                                }
                                onChange={handleInstitutionNamesChange}
                                renderInput={(params) => (
                                    <TextField {...params} label="Наименование учреждения" variant="outlined"/>
