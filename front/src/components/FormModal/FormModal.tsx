...
const FormModal: React.FC<FormModalProps> = ({show, onClose, onSave,onSaveData, patient,form}) => {
    ...
    const [MKB10Value,setMKB10Value] = React.useState<MKB10[]>([]);
    const [institutionsValue,setInstitutionsValue] = React.useState<Institutions[]>([]);
...
// Фетч для тэгов
    ...
    useEffect(() => {
        axios.get(`http://localhost:8080/api/mkb_10/find_all_mkb10s}`)
            .then(response => {
                console.log("МКБ: ",response.data);
                setMKB10Value(response.data.map((mkb10: MKB10) => mkb10));
            })
    ...
    useEffect(() => {
        axios.get(`http://localhost:8080/api/institution_name/find_all_institutions}`)
            .then(response => {
                console.log("Институты: ",response.data);
                setInstitutionsValue(response.data.map((institutions: Institutions) => institutions));
            })
            .catch(error => console.error('Ошибка при загрузке данных:', error));
                    ...
                    >
                        ...
                        <Grid item xs={6}>
                            ...
                            <Autocomplete
                                disablePortal
                                id="combo-box-demo"
                                options={optionDataInstitution}
                                renderInput={(params) => <TextField {...params} label="Институт"/>}
                                // value={formData?.inst_name ? String(formData.inst_name) : ''}
                                // onChange={(event, newValue) => handleChangeTextAutoComplete(event, newValue, 'inst_name')}
                            />
