...
const App: React.FC = () => {
    ...
    const refreshPatientsList = async () => {
        console.log("refreshPatientsList начата");
        try {
            const response = await axios.get(`http://localhost:8080/api/patients/find_all`);
            console.log("refreshPatientsList: данные получены", response.data);
            setPatients(response.data);
            console.log("refreshPatientsList: состояние обновлено");
        } catch (error) {
            console.error("refreshPatientsList: ошибка при загрузке списка пациентов", error);
        }
        console.log("refreshPatientsList в App завершил выполнение");
    };
    console.log("refreshPatientsList в App:", refreshPatientsList);
